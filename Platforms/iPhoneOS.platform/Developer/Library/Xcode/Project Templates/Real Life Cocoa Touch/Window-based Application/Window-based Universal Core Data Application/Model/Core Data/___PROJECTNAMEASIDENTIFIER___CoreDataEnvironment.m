...
{
    if (self = [super init]) {
        self.managedObjectContextHash = [NSMutableDictionary dictionaryWithCapacity:___PROJECTNAMEASIDENTIFIER___CoreDataEnvironmentContextIdentifierCount];
        self.dispatchQueueHash = [NSMutableDictionary dictionaryWithCapacity:___PROJECTNAMEASIDENTIFIER___CoreDataEnvironmentContextIdentifierCount];
...
- (dispatch_queue_t)dispatchQueueForContextIdentifier:(___PROJECTNAMEASIDENTIFIER___CoreDataEnvironmentContextIdentifier)contextIdentifier;
{
    ...
    if (contextIdentifier == ___PROJECTNAMEASIDENTIFIER___CoreDataEnvironmentContextIdentifierMainThreadContext)
...
{
    return [self managedObjectContextForContextIdentifier:___PROJECTNAMEASIDENTIFIER___CoreDataEnvironmentContextIdentifierMainThreadContext];
}
...
//*****************************************************************************
#pragma mark -
#pragma mark ** Test Only Accesssors **
...
- (void)setIsTestEnvironment:(BOOL)newValue
{
    ...
    if (_isTestEnvironment) {
        ...
        self.modelBundle = [NSBundle bundleWithIdentifier:@"com.6Tringle LLC.UnitTest"];
    } else {
        self.storeType = nil;
...
{
    // In practice we don't really need retain autorelease for this accessor.
    // However, as an example, if you have your own custom accessors, you 
    // would need them if you are using the atomic property flags
    @synchronized (self)
    {
        if (_modelBundle)
            return [[_modelBundle retain] autorelease];
        return [[[NSBundle mainBundle] retain] autorelease];
    }
}

- (void)setModelBundle:(NSBundle *)newValue;
{
    @synchronized (self)
    {
        if (newValue != _modelBundle)
        {
            [_modelBundle release];
...
- (NSString *)storeType;
{
    ...
    {
        if (_storeType)
            return [[_storeType retain] autorelease];
        return NSSQLiteStoreType;
//        return NSInMemoryStoreType;  //InMemory can be used for development/debugging/automated tests
    }
}

- (void)setStoreType:(NSString *)newValue;
{
    @synchronized (self)
    {
        if (newValue != _storeType)
        {
            [_storeType release];
            _storeType = newValue;
            [_storeType retain];
